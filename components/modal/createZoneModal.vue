<template>
  ...
  >
    <div
      class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-lg mx-4"
    >
      <div
        class="flex justify-between items-center pb-3 border-b dark:border-gray-700"
      >
      ...
      <form class="mt-4 space-y-4" @submit.prevent="handleSubmit">
        <div>
          <label
            ...
            >Nombre de la Zona <span class="text-red-500">*</span></label
          >
          <input
            id="zoneName"
            v-model="form.name"
        ...
        <div>
          <label
            for="branchId"
            class="block text-sm font-medium text-gray-700 dark:text-gray-300"
            >Sucursal Asociada <span class="text-red-500">*</span></label
          >
          <select
            id="branchId"
          ...
          <p v-if="branchFetchError" class="text-sm text-red-500 mt-1">
            {{ branchFetchError }}
          </p>
        </div>
        ...
        <div>
          <label
            for="zoneManager"
            class="block text-sm font-medium text-gray-700 dark:text-gray-300"
            >Encargado de Zona <span class="text-red-500">*</span></label
          >
          <select
            id="zoneManager"
          ...
          <p v-if="userFetchError" class="text-sm text-red-500 mt-1">
            {{ userFetchError }}
          </p>
        </div>
        ...
        <div>
          <label
            for="zoneDirection"
            class="block text-sm font-medium text-gray-700 dark:text-gray-300"
            >Dirección/Ubicación Específica (Opcional)</label
          >
          <input
            id="zoneDirection"
        ...
        <div class="pt-4 flex justify-end space-x-3">
          ...
          <button
            ...
            :disabled="
              loading || !form.name || !form.branch_id || !form.zone_manager
            "
          >
...
const handleSubmit = async () => {
  if (!form.name || !form.branch_id || !form.zone_manager) {
    errorMessage.value =
      "Nombre de zona, sucursal y encargado son obligatorios.";
    return;
  }
